...
class Client
{
    ...
    public function __construct($config = [])
    {
        ...
        $handlerStack->push(
            Middleware::mapResponse(function (ResponseInterface $response) {
                return FonerodResponse::createFrom($response);
            }),
            'json_response'
        );
    ...
    public function request($method, $params = [])
    {
        try {
            ...
            if ($response->hasError()) {
                ...
                throw new Exceptions\FonerodException($response->error());
            }
            ...
            if (
                ...
                throw new Exceptions\FonerodException($exception->getResponse()->error());
            }
    ...
    /**
     * Makes async request to Fonero Core.
     *
     * @param string        $method
     * @param mixed         $params
    ...
    protected function asyncFulfilled(ResponseInterface $response, callable $callback = null)
    {
        $error = null;
        if ($response->hasError()) {
            $error = new Exceptions\FonerodException($response->error());
        }
    ...
    protected function asyncRejected(RequestException $exception, callable $callback = null)
    {
        if (
            ...
            $exception = new Exceptions\FonerodException(
    ...
    /**
     * Converts amount from satoshi to fonero.
     *
     * @param int $amount
     *
     * @return float
    ...
    /**
     * Converts amount from fonero to satoshi.
     *
     * @param float $amount
     *
     * @return int
